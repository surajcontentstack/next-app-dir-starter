...
export const isBasicConfigValid = () => {
  const requiredVariables = ['CONTENTSTACK_API_KEY', 'CONTENTSTACK_DELIVERY_TOKEN', 'CONTENTSTACK_ENVIRONMENT'];
...
export const isLpConfigValid = () => {
  const requiredVariables = ['CONTENTSTACK_LIVE_PREVIEW', 'CONTENTSTACK_MANAGEMENT_TOKEN', 'CONTENTSTACK_API_HOST', 'CONTENTSTACK_APP_HOST'];
  const missingVariables = requiredVariables.filter(variable => !process.env[variable]);
  if (missingVariables.length > 0) {
    throw new Error(`Missing required environment variables for live preview: ${missingVariables.join(', ')}`);
  }
};
// set region
